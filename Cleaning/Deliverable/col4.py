...
if __name__ == "__main__":
	...
	lines = lines.filter(lambda x: x!=header).map(lambda x: (x[3]))
	
	column4 = lines.map(lambda x: (x, basetype_date(x), semantictype_date(x), validity_date(x)))
						
	column4.saveAsTextFile("column4.out")

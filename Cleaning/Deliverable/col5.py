...
if __name__ == "__main__":
	...
	lines = lines.filter(lambda x: x!=header).map(lambda x: (x[4]))
	
	column5 = lines.map(lambda x: (x, basetype_time(x), semantictype_time(x), validity_time(x)))
						
	column5.saveAsTextFile("column5.out")	
